...
class GenerateOpenApiSpec extends AnyWordSpec with Matchers {
  ...
  "Parse CSV into OpenAPI list" in {
    ...
    val apis : Seq[(_, OpenAPI)] = GenerateOpenApi.fromCsvToOpenAPI(csvFile.bufferedReader(), testPlatform)
  ...
  "Parse CSV into OpenAPI Specification content" in {
    ...
    val contents = GenerateOpenApi.fromCsvToOasContent(csvFile.bufferedReader(), testPlatform)
